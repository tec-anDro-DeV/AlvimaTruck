...
class CircleImageView : AppCompatImageView {
    ...
    @Deprecated(
        "Use {@link #setCircleBackgroundColor(int)} instead",
        ReplaceWith("circleBackgroundColor = context.resources.getColor(circleBackgroundRes)")
    )
    ...
    private fun inTouchableArea(x: Float, y: Float): Boolean {
        return if (mBorderRect.isEmpty) {
            true
        } else (x - mBorderRect.centerX().toDouble()).pow(2.0) + (y - mBorderRect.centerY()
